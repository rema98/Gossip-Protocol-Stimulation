...
//Initializing all message types that are sent and recieved by actors
...
let mutable nodes = int (string (fsi.CommandLineArgs.GetValue 1)) // number of nodes
let topology = string (fsi.CommandLineArgs.GetValue 2) // Topology - gossip or push-sum
let protocol = string (fsi.CommandLineArgs.GetValue 3) // Protocol to use - line, 3D or Imperfect3D
let timer = Diagnostics.Stopwatch() // TImer to check convergence time of algorithm
let system = ActorSystem.Create("System")

// We want a perfect cube for 3D and Imperfect 3D. Hence we take the nearest number of nodes which has a perfect cuberoot
...
// Calculates convergence time. Activates the first actor. Has a counter to ensure all nodes recieved message specified number of times.
    ...
    let rec parentLoop()= actor{
        ...
        | CompletedMessage _ -> 
            ...
            if count = AllNodes then
                ...
                printfn "-----------------------------------------------------------"
                printfn "All %i nodes heard the Message!" AllNodes
                printfn "-----------------------------------------------------------"
                Environment.Exit(0)
        | PushSumResult (sum, weight) ->
            ...
            if count = AllNodes then
                ...
                printfn "Convergence Time: %f ms" timer.Elapsed.TotalMilliseconds
                printfn "-----------------------------------------------------------"
                printfn "All %i nodes converged to the sum!" AllNodes
                printfn "-----------------------------------------------------------"
                Environment.Exit(0)
...
let ChildActor(mailbox: Actor<_>) =
    ...
    let rec childLoop()= actor{
        ...
        | ActivateChildActor ->
            // Find a random neighbor and send message if the neighbor is active.
        ...
        | CallChildActor ->
            // Activate itself if the actor hasn't recieved any message
            if messageCount = 0 then 
                mailbox.Self <! ActivateChildActor
            // Inform parent when the actor revieved message 10 times. Else, increase counter.
...
| "line" ->
    ...
    // Spawn actors based on number of nodes
    for x in [0..nodes] do
    ...
    // Add reference of neigbours for an actor in neighbourArray
    for i in [ 0 .. nodes ] do
...
//Ignore Any other input from user
